...
const VideoMeeting = ({ appointmentId, isDoctor, backendUrl, dtoken, onEndMeeting }) => {
  ...
  const endMeeting = async () => {
    try {
      // If doctor, send end meeting request to backend
      if (isDoctor && room) {
        await axios.post(
    ...
    <div className="bg-gray-50 rounded-lg overflow-hidden shadow-md">
        ...
        <div className="relative bg-black rounded-lg overflow-hidden h-[300px]">
          ...
          {remoteParticipant && remoteVideoTracks.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-800 text-white">
              <div className="text-center">
                <div className="text-5xl mb-2">👤</div>
                <p>Patient camera is turned off</p>
              </div>
            </div>
          )}
          {remoteParticipant && (
      ...
      <div className="p-4 bg-white border-t border-gray-200">
        ...
        <p className="text-sm text-gray-600">
          You are the doctor in this consultation.
        </p>
        <p className="text-sm text-gray-600">
          Appointment ID: {appointmentId}
        </p>
        <p className="text-sm text-gray-600 mt-2">
          <strong>Note:</strong> When you end the meeting, the appointment will be marked as completed.
        </p>
      </div>
